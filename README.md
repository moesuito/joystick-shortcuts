...
- **Chord bindings** — pick any button as a "hold modifier" (including the Xbox/Guide button) and the binding only fires when the modifier is held *plus* the trigger button is pressed. Lets you reuse the same trigger button for multiple actions.
...
Supported buttons: `A`, `B`, `X`, `Y`, `LB`, `RB`, `LT`, `RT`, `BACK`, `START`, `LS` (click), `RS` (click), the D-Pad (`UP` / `DOWN` / `LEFT` / `RIGHT`), and `GUIDE` (the Xbox button — read via the undocumented `XInputGetStateEx`).
...
- **Xbox/Guide button is shared with Steam.** The Guide button is read via the undocumented `XInputGetStateEx` (ordinal 100), the same API Steam itself uses. We can read the press, but we **cannot block** it from also reaching Steam — there is no way to suppress the gamepad signal at user level. So a chord like `GUIDE + A` will fire your custom action *and* Steam may still pop Big Picture / overlay. If that bothers you, either (a) disable "Open Big Picture on Guide press" in Steam settings, or (b) pick a different modifier (`BACK`, `START`, etc.) that no other process grabs.
